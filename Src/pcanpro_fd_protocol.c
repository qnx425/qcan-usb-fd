...
void pcan_ep0_receive( void )
{
  if( drv_load_packet[0] == 0 )
  {
    ...
    pcan_led_set_mode( LED_STAT, LED_MODE_ON, 0xFFFFFFFF );
  }
  else
...
{
  uint8_t  msgSize = MCP251XFD_DLCToByte( pmsg->size, true );

  struct ucan_rx_msg *pcan_msg = pcan_data_alloc_buffer( UCAN_MSG_CAN_RX, sizeof(struct ucan_rx_msg) + msgSize );
  if( !pcan_msg )
    return -1;
  ...
  if( !(pmsg->flags & MSG_FLAG_ECHO) )
  {
    ...
    {
      pcan_led_set_mode( LED_STAT, LED_MODE_BLINK_FAST, 237 );
    }
  }
  ...
  memcpy( pcan_msg->d, pmsg->data, msgSize );
  return 0;
}
...
int pcan_protocol_tx_frame( struct ucan_tx_msg *pmsg )
{
  ...
  if( msg.size ) {
	  memcpy( msg.data, pmsg->d, MCP251XFD_DLCToByte( msg.size, true ) );
  }
...
static void pcan_protocol_process_cmd( uint8_t *ptr, uint16_t size )
{
  ...
  while( size >= sizeof( struct ucan_command ) )
  {
	...
	if( opcode < 18 ) {
		printf( "%10u %s \r\n", pcan_timestamp_us(), UcanCmdStr[opcode] );
	}
	else if( opcode == UCAN_USB_CMD_CLK_SET ) {
		printf( "%10u %s \r\n", pcan_timestamp_us(), UcanCmdClkSet );
	}
	else if( opcode == UCAN_CMD_END_OF_COLLECTION ) {
		printf( "%10u %s \r\n", pcan_timestamp_us(), UcanCmdEndColl );
	}
	else if( opcode == UCAN_USB_CMD_DEVID_SET ) {
		printf( "%10u %s \r\n", pcan_timestamp_us(), UcanCmdDevidSet );
	}
	else {
		printf( "%10u %d \r\n", pcan_timestamp_us(), opcode );
	}
#endif
    switch( UCAN_CMD_OPCODE( pcmd ) )
    {
...
#ifdef DEBUG
          printf( "%10u slow - brp: %d, tseg1: %d, tseg2: %d, sjw: %d\r\n", pcan_timestamp_us(), ptiming->brp, ptiming->tseg1, ptiming->tseg2, ptiming->sjw_t );
#endif
...
#ifdef DEBUG
          printf( "%10u fast - brp: %d, tseg1: %d, tseg2: %d, sjw: %d\r\n", pcan_timestamp_us(), ptiming->brp, ptiming->tseg1, ptiming->tseg2, ptiming->sjw );
#endif
          pcan_can_set_bitrate_ex(	  channel,
        		  	  	  	  	  	  ptiming->brp,
									  ptiming->tseg1,
...
#ifdef DEBUG
          printf( "%10u opt_mask: 0x%04X \r\n", pcan_timestamp_us(), pcan_device.can[channel].opt_mask );
#endif
        }
        break;
      ...
      case UCAN_USB_CMD_CLK_SET:
        if( channel < CAN_CHANNEL_MAX )
        {
          ...
          if( pclock->mode == UCAN_USB_CLK_40MHZ || pclock->mode == UCAN_USB_CLK_80MHZ ) {
        	  pcan_device.can[channel].can_clock = pclock->mode;
          }
#ifdef DEBUG
          printf( "%10u can_clock: %d \r\n", pcan_timestamp_us(), pcan_device.can[channel].can_clock );
#endif
      ...
      case UCAN_USB_CMD_LED_SET:
        if( channel < CAN_CHANNEL_MAX )
        {
          ...
          pcan_led_set_mode( LED_STAT, pled->mode, 0xFFFFFFFF );
        }
        break;
